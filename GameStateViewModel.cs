...
public record KingdomState(CardPile[] Supply, CardInstanceDto[] Trash);
public record CardPile(int CardId, int Count);
public record CardInstanceDto(string InstanceId, int CardId);
public record FullPlayerData(string PlayerId, CardInstanceDto[] Hand, int DeckCount, CardInstanceDto[] Discard, CardInstanceDto[] Play, PlayerResources Resources, CardFilter? ActiveCardChoice);
...
public static partial class GameStateExtensions
{
  ...
  public static GameStateViewModel ToPlayerGameStateViewModel(this GameState @this, string playerId)
  {
    ...
    return new GameStateViewModel(
      GameId: @this.GameId,
      KingdomState: new KingdomState([.. @this.KingdomCards.Select(kc => new CardPile(kc.Card.Id, kc.Remaining))], [.. @this.Trash.Select(card => new CardInstanceDto(card.Id, card.Card.Id))]),
      TurnState: new TurnState(@this.Players[@this.CurrentPlayer].Id, @this.Players[@this.ActivePlayer].Id, @this.CurrentTurn, @this.Phase.ToString()),
      Log: new LogState(@this.Log),
      Me: new FullPlayerData(playerId, [.. player.Hand.Select(ToDto)], player.Deck.Length, [.. player.Discard.Select(ToDto)], [.. player.Play.Select(ToDto)], player.Resources, player.ActiveFilter),
      Opponents: [
